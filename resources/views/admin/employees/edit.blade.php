                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('admin.employees.update', $employee) }}" class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="department" value="{{ old('department', $employee->department) }}" class="w-full border-gray-300 rounded" />
                        </div>
                        <div>
                            <label class="block text-sm mb-1">Designation</label>
                            <input type="text" name="designation" value="{{ old('designation', $employee->designation) }}" class="w-full border-gray-300 rounded" />
                        </div>
                        <div>
                            <label class="block text-sm mb-1">Biometric ID</label>
                            <input type="text" name="biometric_id" value="{{ old('biometric_id', $employee->biometric_id) }}" class="w-full border-gray-300 rounded" />
                        </div>
                        <div>
                            <label class="block text-sm mb-1">Leave Quota</label>
                            <input type="number" name="leave_quota" value="{{ old('leave_quota', $employee->leave_quota) }}" class="w-full border-gray-300 rounded" />
                        </div>
                        <div>
